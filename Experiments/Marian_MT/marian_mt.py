...
from transformers.modeling_bart import SinusoidalPositionalEmbedding, LayerNorm, _make_linear_from_emb
from transformers.modeling_bart import _reorder_buffer, ACT2FN, invert_mask, Attention
import torch
import random
...
class DecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.final_layer_norm = LayerNorm(self.embed_dim)
        # self.cross_attention = nn.init.normal_()
    ...
    def forward(
        ...
        x, enc_att = self.encoder_attn(
            query=x,
            key=encoder_hidden_states,
            key_padding_mask=encoder_attn_mask,
            layer_state=layer_state,  # mutates layer state
            output_attentions=output_attentions # only change made
        )
        self.cross_attention = enc_att
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        if not self.normalize_before:
            x = self.encoder_attn_layer_norm(x)
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #isidora - start - set to True in any case
        ...
        """
        #isidora - start - return dec_enc_dec_attn instead of all decoder properties
        dec_enc_dec_attn = self.decoder(
            decoder_input_ids,
            encoder_outputs[0],
            attention_mask,
            decoder_padding_mask,
        ...
        if not return_dict:
            ...
            #isidora - start - return dec_enc_dec_attn instead of decoder_outputs + encoder_outputs
            return dec_enc_dec_attn
            #isidora - end
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def forward(
        ...
        **unused,
        ...
        #isidora - start - return enc_dec_attn instead of model outputs
        enc_dec_attn = self.model(
            input_ids,
            attention_mask=attention_mask,
            decoder_input_ids=decoder_input_ids,
            encoder_outputs=encoder_outputs,
